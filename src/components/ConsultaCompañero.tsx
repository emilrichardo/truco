...
export function ConsultaCompañero({
  ...
  // Estado colapsado: sólo un icono "?" para reabrir (sin avatar — el
  // avatar duplicaba al del compañero que ya está visible en su puesto
  // en la mesa).
  if (!abierto) {
    return (
      <div className="absolute z-[800] right-2 top-32 sm:top-36 sm:right-6 pointer-events-none">
        <button
          ...
          className="pointer-events-auto w-10 h-10 rounded-full bg-dorado text-carbon text-xl font-bold flex items-center justify-center shadow-2xl border-2 border-carbon hover:scale-110 active:scale-95 transition animate-bounce"
        >
          ?
        </button>
      </div>
